...
export default function Analysis6View() {
  const initMap = () => {
  ...
  // 执行多边形裁剪分析
  const clipByPolyAnalysis = () => {
    //清除之前的分析结果
    clearA();
    //显示进度条
    startPressBar();
    let resultname =
      resultBaseUrl + 'clipByPolyAnalysisResultLayer' + getCurentTime();
    //实例化ClipByPolygon类develop.smaryun.com
    // eslint-disable-next-line
    let clipParam = new Zondy.Service.ClipByPolygon({
      //访问IGServer的IP
    ...
    //多边形点坐标串
    clipParam.strPos =
      '0.46, 30.1,11.48, 6.22,36.73, 7.6,58.77, 25.51,41.33, 49.39, 0.46, 30.1';
