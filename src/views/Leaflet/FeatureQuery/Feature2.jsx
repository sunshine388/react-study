...
export default function Feature2View() {
  ...
  const query = () => {
    ...
    //创建一个用于查询的圆
    // eslint-disable-next-line
    let pointObj = new Zondy.Object.Point2D(116.4375, 41.53125);
    // eslint-disable-next-line
    let geomObj = new Zondy.Object.Circle(pointObj, 20);
    //实例化查询参数对象
    // eslint-disable-next-line
  ...
  return (
    <React.Fragment>
      <div className='map_container'>
        <div id='preview2' className='preview' style={{ display: 'none' }}>
          <div className='loading'></div>
          <span>正在查询，请稍候</span>
        </div>
        <div className='ToolLib'>
          <input type='button' value='固定圆查询' onClick={query} />
        </div>
        <div>
          该示例为矢量地图文档固定圆查询(返回查询结果JSON)接口示例，请注意：
          <br />
          &nbsp;&nbsp;&nbsp;&nbsp;1、默认查询示例数据库（OpenLayerVecterMap.hdf）下“世界地图经纬度”中的简单要素类“世界政区”，操作前请先确认此数据库已经附加，地图文档（WorldJWVector）已经发布；
          <br />
          &nbsp;&nbsp;&nbsp;&nbsp;2、用于查询的固定圆以(116.4375,
          41.53125)点为圆心，以20为半径构成，点击【固定圆查询】按钮执行查询。
          <br />
        </div>
